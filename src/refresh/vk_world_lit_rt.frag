...
vec3 reflection_color(vec3 normal, vec3 base)
{
    vec3 view_direction = normalize(v_position - pc.dlight.xyz);
    vec3 direction = reflect(view_direction, normal);
    rayQueryEXT query;
    rayQueryInitializeEXT(query, scene, gl_RayFlagsOpaqueEXT, 0xff,
        v_position + normal * 0.05, 0.05, direction, 8192.0);
    while (rayQueryProceedEXT(query)) { }
    bool miss = rayQueryGetIntersectionTypeEXT(query, true) ==
        gl_RayQueryCommittedIntersectionNoneEXT;
    vec3 environment = pc.fog.a != 0.0 ? pc.fog.rgb : vec3(0.12, 0.16, 0.22);
    vec3 hit = base * mix(0.55, 0.2, pc.rt_roughness);
    return miss ? environment : hit;
}
...
void main()
{
    ...
    if (mode > 1.5) {
        ...
        if (pc.rt_enabled > 0.5 && pc.rt_specular > 0.0) {
            vec3 reflected = reflection_color(normal, out_color.rgb);
            float strength = pc.rt_specular * (1.0 - 0.75 * pc.rt_roughness);
            out_color.rgb = mix(out_color.rgb, reflected, clamp(strength, 0.0, 1.0));
        }

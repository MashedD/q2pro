...
enum {
    ...
    VK_TIMESTAMP_FSR_END,
    VK_TIMESTAMP_FRAMEGEN_BEGIN,
    VK_TIMESTAMP_FRAMEGEN_END,
    VK_TIMESTAMP_PRESENT_BEGIN,
    VK_TIMESTAMP_PRESENT_END,
};
...
;
static const uint32_t vk_motion_camera_vert_spv[] =
#include "vk_motion_camera_vert_spv.h"
;
static const uint32_t vk_motion_camera_frag_spv[] =
#include "vk_motion_camera_frag_spv.h"
;
typedef struct {
    float rect[4];
    float color[4];
...
typedef struct {
    ...
    VkDescriptorSet fsr_descriptor_set;
    uint32_t width;
    uint32_t height;
    uint32_t mip_levels;
} vk_texture_t;
...
typedef struct {
    ...
    VkPipeline fsr_motion_alias_reactive_pipeline;
    VkPipeline fsr_motion_world_reactive_pipeline;
    VkPipeline fsr_motion_color_pipeline;
    VkPipeline fsr_motion_camera_pipeline;
    ...
    bool swapchain_mutable_format_supported;
    ...
    entity_t fsr_history[MAX_ENTITIES];
    entity_t fsr_pending_history[MAX_ENTITIES];
    ...
    VkDeviceSize particle_motion_offset;
    uint32_t particle_motion_ranges[5];
    struct {
        mat4_t mvp;
    ...
    vk_texture_t fsr_texture_views[MAX_RIMAGES]; // non-owning descriptor aliases
...
static bool vk_fsr_user_requested(void)
{
    return r_fsr && r_fsr->integer != 0;
}

static bool vk_fsr_auto_enabled(void)
{
    ...
    if (!Q_stricmp(mode, "fast")) {
        if (frame_generation) {
            if (!vk.fsr_motion_override_logged) {
                ...
                vk.fsr_motion_override_logged = true;
            }
            return false;
        }
        vk.fsr_motion_override_logged = false;
        return true;
    }
    if (!Q_stricmp(mode, "full")) {
        vk.fsr_motion_override_logged = false;
        return false;
    }
...
static bool vk_fsr_auto_is_faster(uint64_t native, uint64_t fsr)
{
    return native && fsr && fsr <= native - (native + 19) / 20;
}

/* Called once for a completed submission, with CPU and GPU costs from that
                                ...
                                uint32_t generation, bool eligible)
{
    if (!vk_fsr_auto_enabled() || !vk_fsr_user_requested() || !eligible ||
        ...
        return;
    uint64_t frame_usec = max(record_usec, gpu_usec);
    if (!frame_usec)
        return;
    if (vk.fsr_auto_warmup) {
        vk.fsr_auto_warmup--;
        return;
    }
    vk.fsr_auto_cpu_only = !gpu_usec;
    vk.fsr_auto_window[vk.fsr_auto_samples++] = frame_usec;
    if (vk.fsr_auto_samples < VK_FSR_AUTO_SAMPLE_COUNT)
        return;
    uint64_t average = vk_fsr_auto_average(vk.fsr_auto_window,
                                           vk.fsr_auto_samples);
    vk.fsr_auto_samples = 0;
    if (vk.fsr_auto_phase == VK_FSR_AUTO_WARMUP) {
        vk.fsr_auto_native_usec = average;
        vk.fsr_auto_phase = VK_FSR_AUTO_TRIAL;
        vk.fsr_auto_warmup = 15;
        vk.fsr_auto_generation++;
        vk.fsr_auto_recreate = true;
        Com_Printf("Vulkan FSR3 auto: native=%lluus; testing selected %s (%s)\n",
                   (unsigned long long)average,
                   vk_fsr_quality_name(vk.fsr_auto_quality),
                   vk.fsr_auto_cpu_only ? "CPU-only" : "CPU/GPU");
        return;
    }
    vk.fsr_auto_fsr_usec = average;
    bool accepted = vk_fsr_auto_is_faster(vk.fsr_auto_native_usec, average);
    vk.fsr_auto_phase = accepted ? VK_FSR_AUTO_ACCEPTED : VK_FSR_AUTO_REJECTED;
    vk.fsr_auto_recreate = !accepted;
    Com_Printf("Vulkan FSR3 auto: native=%lluus %s=%lluus (%s); %s\n",
               (unsigned long long)vk.fsr_auto_native_usec,
               vk_fsr_quality_name(vk.fsr_auto_quality),
               (unsigned long long)average,
               vk.fsr_auto_cpu_only ? "CPU-only" : "CPU/GPU",
               accepted ? "keeping selected quality" :
                          "less than 5% faster; using native");
}
...
static VkExtent2D vk_fsr_render_extent(void)
{
    ...
    /* Keep compute and attachment dimensions well behaved on odd display
     * sizes, while never allowing the reduced target to become zero. */
    extent.width = max((extent.width / 2) * 2, 1);
    extent.height = max((extent.height / 2) * 2, 1);
    return extent;
}
...
static bool vk_create_impact_texture(void);
...
static const vk_texture_t *vk_texture_for_index(unsigned index, bool allow_nobind)
{
    ...
    if (vk.frame_fsr && (vk.active_render_pass == vk.render_pass ||
                         vk.active_render_pass == vk.fsr_motion_render_pass) &&
        vk.textures[index].fsr_descriptor_set) {
        vk.fsr_texture_views[index] = vk.textures[index];
        vk.fsr_texture_views[index].descriptor_set = vk.textures[index].fsr_descriptor_set;
        return &vk.fsr_texture_views[index];
    }
...
static bool vk_image_barrier_batch_add(vk_image_barrier_batch_t *batch,
                                        ...
                                        VkPipelineStageFlags dst_stage)
{
    if (!image || (old_layout == new_layout && new_layout != VK_IMAGE_LAYOUT_GENERAL))
        return false;
    if (batch->count == q_countof(batch->barriers))
                                                      ...
                                                      VkSampler sampler)
{
    ...
    vk.UpdateDescriptorSets(vk.device, 1, &write, 0, NULL);
    if (texture->fsr_descriptor_set) {
        write.dstSet = texture->fsr_descriptor_set;
        if (sampler == vk.sampler && vk.fsr_sampler)
            image_info.sampler = vk.fsr_sampler;
        vk.UpdateDescriptorSets(vk.device, 1, &write, 0, NULL);
    }
}
...
static bool vk_create_image_target(vk_texture_t *texture, uint32_t width,
                                   ...
                                   VkImageUsageFlags usage)
{
    ...
    uint32_t memory_type = vk_find_memory_type(req.memoryTypeBits,
                                               VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT);
    if (memory_type == UINT32_MAX) {
        Com_SetLastError("No suitable Vulkan image target memory type");
        goto fail;
    }
...
{
    return vk_create_image_target(texture, width, height, format,
...
static void vk_destroy_fsr_resources(void)
{
    ...
    vk.fsr_pause_layout = VK_IMAGE_LAYOUT_UNDEFINED;
...
static bool vk_create_fsr_resources(void)
{
    ...
    if (sizeof(vk_fsr_motion_push_t) >
        vk.physical_device_properties.limits.maxPushConstantsSize) {
        Com_WPrintf("Vulkan FSR3 motion data exceeds device push constant limit; using native resolution\n");
    ...
    if (vk.fsr_swapchain_direct &&
        !vk_create_image_target(&vk.fsr_pause_texture,
                                vk.swapchain_extent.width,
                                vk.swapchain_extent.height,
                                vk.fsr_output_format,
                                VK_IMAGE_USAGE_SAMPLED_BIT |
                                VK_IMAGE_USAGE_TRANSFER_DST_BIT)) {
        Com_WPrintf("Couldn't create Vulkan FSR3 direct-output pause cache: %s; using intermediate output\n",
                    Com_GetLastError());
        vk.fsr_swapchain_direct = false;
        if (!vk_create_image_target(&vk.fsr_output_texture,
                                    vk.swapchain_extent.width,
                                    vk.swapchain_extent.height,
                                    vk.fsr_output_format,
                                    VK_IMAGE_USAGE_SAMPLED_BIT |
    ...
    if (vk.fsr_pause_texture.image)
        vk_update_texture_descriptor_with_sampler(&vk.fsr_pause_texture,
                                                  vk.postprocess_sampler);
    vk.fsr_output_layout = VK_IMAGE_LAYOUT_UNDEFINED;
    vk.fsr_frame_generation_layout = VK_IMAGE_LAYOUT_UNDEFINED;
    vk.fsr_pause_layout = VK_IMAGE_LAYOUT_UNDEFINED;
                                   ...
                                   bool mipmaps)
{
    ...
    result = vk.AllocateDescriptorSets(vk.device, &descriptor_info,
                                       &uploaded.fsr_descriptor_set);
    if (result != VK_SUCCESS) {
        vk_fail_result("vkAllocateDescriptorSets(FSR sampler)", result);
        goto fail;
    }
...
static void vk_destroy_texture_resource(vk_texture_t *texture)
{
    ...
    if (texture->descriptor_set && vk.descriptor_pool)
        vk.FreeDescriptorSets(vk.device, vk.descriptor_pool, 1,
                              &texture->descriptor_set);
    if (texture->fsr_descriptor_set && vk.descriptor_pool)
        vk.FreeDescriptorSets(vk.device, vk.descriptor_pool, 1,
                              &texture->fsr_descriptor_set);
    if (texture->view)
...
static bool vk_has_device_extension(VkPhysicalDevice device, const char *name)
{
    uint32_t count = 0;
    VkResult result;
...
{
    return vk_has_device_extension(device, VK_KHR_SWAPCHAIN_EXTENSION_NAME);
}
...
static bool vk_pick_physical_device(void)
{
    ...
    vk.swapchain_mutable_format_supported = vk_has_device_extension(
        vk.physical_device, VK_KHR_SWAPCHAIN_MUTABLE_FORMAT_EXTENSION_NAME);
...
static bool vk_create_device(void)
{
    ...
    features.independentBlend = vk.physical_device_features.independentBlend;
...
static bool vk_create_sampler(VkSampler *sampler)
{
    ...
    VkSamplerCreateInfo sampler_info = {
        ...
        .mipLodBias = sampler == &vk.fsr_sampler ? vk.fsr_sampler_bias : 0.0f,
...
static void vk_texturemode_changed(cvar_t *self)
{
    ...
    if (vk.fsr_sampler)
        vk.DestroySampler(vk.device, vk.fsr_sampler, NULL);
    vk.fsr_sampler = VK_NULL_HANDLE;
    vk_create_sampler(&vk.fsr_sampler);
...
static bool vk_create_timestamp_query_pool(void)
{
    VkQueryPoolCreateInfo info = {
        ...
        .queryCount = VK_MAX_FRAMES_IN_FLIGHT * VK_TIMESTAMP_QUERY_COUNT,
    };
    ...
    memset(vk.timestamp_scope_mask, 0, sizeof(vk.timestamp_scope_mask));
    return true;
}

static bool vk_fsr_gpu_timing_enabled(void)
{
    return vk_perf_stats && vk_perf_stats->integer != 0;
}

static void vk_write_timestamp(uint32_t query)
{
    if (!vk.timestamp_query_pool)
        return;
    vk.CmdWriteTimestamp(vk.command_buffers[vk.current_image],
                         VK_PIPELINE_STAGE_BOTTOM_OF_PIPE_BIT,
                         vk.timestamp_query_pool,
                         vk.frame_index * VK_TIMESTAMP_QUERY_COUNT + query);
}

static void vk_write_fsr_timestamp(uint32_t query)
{
    if (!vk_fsr_gpu_timing_enabled() || !vk.timestamp_query_pool ||
        (vk.timestamp_scope_mask[vk.frame_index] & (1u << query)))
        return;
    vk_write_timestamp(query);
    vk.timestamp_scope_mask[vk.frame_index] |= (uint16_t)(1u << query);
}

static bool vk_read_timestamp_pair(uint32_t first, unsigned *usec)
{
    uint64_t ticks[2];
    VkResult result = vk.GetQueryPoolResults(vk.device,
                                              vk.timestamp_query_pool,
    ...
    double elapsed = (double)(ticks[1] - ticks[0]) *
        vk.physical_device_properties.limits.timestampPeriod / 1000.0;
    *usec = min(elapsed, (double)UINT_MAX);
    return true;
}
...
static bool vk_create_frame_resources(void)
{
    ...
    const uint32_t texture_descriptor_count =
        /* Base/FSR sampler pairs, cubemap faces, scratch uploads and targets. */
        MAX_RIMAGES * 2 + VK_MAX_CUBEMAPS * 12 + 64;
...
static void vk_destroy_swapchain(void)
{
    ...
    if (vk.fsr_motion_camera_pipeline) {
        vk.DestroyPipeline(vk.device, vk.fsr_motion_camera_pipeline, NULL);
        vk.fsr_motion_camera_pipeline = VK_NULL_HANDLE;
    }
    if (vk.fsr_motion_alias_reactive_pipeline) {
        vk.DestroyPipeline(vk.device, vk.fsr_motion_alias_reactive_pipeline, NULL);
        vk.fsr_motion_alias_reactive_pipeline = VK_NULL_HANDLE;
    }
    if (vk.fsr_motion_world_reactive_pipeline) {
    ...
    VkSubpassDependency motion_dependencies[] = {
        {
            .srcSubpass = VK_SUBPASS_EXTERNAL,
            .dstSubpass = 0,
            .srcStageMask = VK_PIPELINE_STAGE_COMPUTE_SHADER_BIT | VK_PIPELINE_STAGE_FRAGMENT_SHADER_BIT | VK_PIPELINE_STAGE_EARLY_FRAGMENT_TESTS_BIT |
                            VK_PIPELINE_STAGE_LATE_FRAGMENT_TESTS_BIT,
            .dstStageMask = VK_PIPELINE_STAGE_COLOR_ATTACHMENT_OUTPUT_BIT | VK_PIPELINE_STAGE_FRAGMENT_SHADER_BIT | VK_PIPELINE_STAGE_EARLY_FRAGMENT_TESTS_BIT |
                            VK_PIPELINE_STAGE_LATE_FRAGMENT_TESTS_BIT,
            .srcAccessMask = VK_ACCESS_SHADER_READ_BIT | VK_ACCESS_DEPTH_STENCIL_ATTACHMENT_WRITE_BIT,
            .dstAccessMask = VK_ACCESS_COLOR_ATTACHMENT_WRITE_BIT | VK_ACCESS_SHADER_READ_BIT | VK_ACCESS_DEPTH_STENCIL_ATTACHMENT_READ_BIT,
        },
        {
            .srcSubpass = 0,
            .dstSubpass = VK_SUBPASS_EXTERNAL,
...
static bool vk_create_scene_target(void)
{
    ...
    struct {
        ...
        { &vk.bloom_texture, &vk.bloom_layout,
          max(vk.render_extent.width / bloom_downsample, 1),
          max(vk.render_extent.height / bloom_downsample, 1) },
...
static bool vk_create_fsr_motion_pipeline(VkPipeline *pipeline,
                                          ...
                                          bool alias)
{
    bool camera = vert_spv == vk_motion_camera_vert_spv;
    bool reactive_only = vert_spv == vk_motion_color_vert_spv ||
    ...
    for (uint32_t i = 0; i < 4; i++)
        bindings[i] = (VkVertexInputBindingDescription) { i, sizeof(vk_vertex_t), VK_VERTEX_INPUT_RATE_VERTEX };
    VkVertexInputAttributeDescription attributes[] = {
        { 0, 0, VK_FORMAT_R32G32B32_SFLOAT, offsetof(vk_vertex_t, position) },
        { 1, 0, VK_FORMAT_R32G32_SFLOAT, offsetof(vk_vertex_t, uv) },
        { 2, 0, VK_FORMAT_R32G32B32A32_SFLOAT, offsetof(vk_vertex_t, color) },
        { 3, 1, VK_FORMAT_R32G32B32_SFLOAT, offsetof(vk_vertex_t, position) },
        { 4, 2, VK_FORMAT_R32G32B32_SFLOAT, offsetof(vk_vertex_t, position) },
        { 5, 3, VK_FORMAT_R32G32B32_SFLOAT, offsetof(vk_vertex_t, position) },
    };
    VkPipelineVertexInputStateCreateInfo vertex_input = {
        .sType = VK_STRUCTURE_TYPE_PIPELINE_VERTEX_INPUT_STATE_CREATE_INFO,
        .vertexBindingDescriptionCount = camera ? 0 : (alias ? 4 : 1),
        .pVertexBindingDescriptions = bindings,
        .vertexAttributeDescriptionCount = camera ? 0 : (alias ? 6 : 3),
        .pVertexAttributeDescriptions = attributes,
    };
    VkPipelineInputAssemblyStateCreateInfo input_assembly = {
        .sType = VK_STRUCTURE_TYPE_PIPELINE_INPUT_ASSEMBLY_STATE_CREATE_INFO,
    ...
    if (reactive_only)
        blend_attachments[0].colorWriteMask = 0;
    if (!camera) {
        ...
        blend_attachments[1].colorBlendOp = VK_BLEND_OP_MAX;
...
{
    if (!vk.fsr3 || vk.sample_count != VK_SAMPLE_COUNT_1_BIT)
        return true;
    ...
    if (!vk_create_fsr_motion_pipeline(&vk.fsr_motion_camera_pipeline,
                                       vk_motion_camera_vert_spv,
                                       sizeof(vk_motion_camera_vert_spv), false) ||
        !vk_create_fsr_motion_pipeline(&vk.fsr_motion_world_pipeline,
        ...
        !vk_create_fsr_motion_pipeline(&vk.fsr_motion_alias_reactive_pipeline,
                                       vk_motion_alias_vert_spv,
                                       sizeof(vk_motion_alias_vert_spv), true) ||
        !vk_create_fsr_motion_pipeline(&vk.fsr_motion_world_reactive_pipeline,
                                       vk_motion_world_vert_spv,
                                       sizeof(vk_motion_world_vert_spv), false)) {
...
static bool vk_create_swapchain(int width, int height)
{
    ...
    VkFormat fsr_format = vk_fsr_output_format(surface_format.format);
    bool mutable_format_pair = fsr_format == surface_format.format;
    if (!mutable_format_pair) {
        for (uint32_t i = 0; i < format_count; i++) {
            if (formats[i].format == fsr_format &&
                formats[i].colorSpace == surface_format.colorSpace) {
                mutable_format_pair = true;
                break;
            }
        }
    }
    ...
    vk.swapchain_storage =
        (caps.supportedUsageFlags & VK_IMAGE_USAGE_STORAGE_BIT) != 0;
    bool direct_candidate = vk_fsr_any_requested() &&
        !vk_fsr_frame_generation_requested() && vk.swapchain_transfer_src &&
        vk.swapchain_storage && mutable_format_pair &&
        (fsr_format == surface_format.format ||
         vk.swapchain_mutable_format_supported);
    ...
    VkFormat view_formats[2] = { surface_format.format, fsr_format };
    VkImageFormatListCreateInfo format_list = {
        .sType = VK_STRUCTURE_TYPE_IMAGE_FORMAT_LIST_CREATE_INFO,
        .viewFormatCount = fsr_format == surface_format.format ? 1u : 2u,
        .pViewFormats = view_formats,
    };

    VkSwapchainCreateInfoKHR create_info = {
        .sType = VK_STRUCTURE_TYPE_SWAPCHAIN_CREATE_INFO_KHR,
        .pNext = direct_candidate && fsr_format != surface_format.format ?
            &format_list : NULL,
        .flags = direct_candidate && fsr_format != surface_format.format ?
            VK_SWAPCHAIN_CREATE_MUTABLE_FORMAT_BIT_KHR : 0,
        .surface = vk.surface,
    ...
    vk.fsr_swapchain_direct = direct_candidate;
    if (vk.separate_presentation && !vk.fsr_swapchain_direct &&
        !vk_fsr_frame_generation_requested() && !vk.fsr_warned) {
        Com_Printf("Vulkan FSR3 direct swapchain output unavailable; using intermediate output\n");
    }
                                ...
                                VkPipelineStageFlags dst_stage)
{
    ...
    } else if (vk.swapchain_layouts[image_index] == VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL) {
        src_stage = VK_PIPELINE_STAGE_TRANSFER_BIT;
...
static void vk_batch_color_target(vk_image_barrier_batch_t *batch,
                                  vk_texture_t *texture, VkImageLayout *layout,
                                  VkImageLayout new_layout, VkAccessFlags dst_access,
                                  VkPipelineStageFlags dst_stage);
...
static void vk_transition_color_target(VkCommandBuffer cmd, vk_texture_t *texture,
                                       ...
                                       VkPipelineStageFlags dst_stage)
{
    vk_image_barrier_batch_t batch = { .cmd = cmd };
    vk_batch_color_target(&batch, texture, layout, new_layout, dst_access, dst_stage);
    vk_image_barrier_batch_submit(cmd, &batch);
}
...
static void vk_batch_color_target(vk_image_barrier_batch_t *batch,
                                  ...
                                  VkPipelineStageFlags dst_stage)
{
    if (!texture->image || (*layout == new_layout && new_layout != VK_IMAGE_LAYOUT_GENERAL))
        return;
...
{
    vk_image_barrier_batch_t batch = { .cmd = cmd };
    vk_batch_depth(&batch, new_layout, dst_access, dst_stage);
    vk_image_barrier_batch_submit(cmd, &batch);
}
...
static void vk_set_3d_viewport(const refdef_t *fd)
{
    ...
    float scale_x = (float)vk.render_extent.width /
        max((float)vk.swapchain_extent.width, 1.0f);
    float scale_y = (float)vk.render_extent.height /
        max((float)vk.swapchain_extent.height, 1.0f);
    int x = Q_clip(Q_rint(fd->x * scale_x), 0, (int)vk.render_extent.width);
    int y = Q_clip(Q_rint(fd->y * scale_y), 0, (int)vk.render_extent.height);
    int x2 = Q_clip(Q_rint((fd->x + width) * scale_x), x,
                    (int)vk.render_extent.width);
    int y2 = Q_clip(Q_rint((fd->y + height) * scale_y), y,
                    (int)vk.render_extent.height);
                                     ...
                                     const entity_t *ent, const vec3_t axis[3])
{
    ...
    vk_entity_model_matrix(model, ent, axis);
    vk_entity_projection_matrix(proj, fd, ent);
    if (vk.frame_fsr && vk.render_extent.width && vk.render_extent.height) {
        proj[8] -= 2.0f * vk.fsr_jitter[0] / vk.render_extent.width;
        proj[9] += 2.0f * vk.fsr_jitter[1] / vk.render_extent.height;
    }
    vk_view_matrix(view, fd);
    vk_matrix_multiply(view_model, view, model);
    vk_matrix_multiply(out, proj, view_model);
...
static const entity_t *vk_fsr_previous_entity(const entity_t *ent)
{
    if (!ent->temporal_id || vk.fsr_reset || !vk.fsr_previous_fd_valid)
        return NULL;
    const entity_t *found = NULL;
    for (uint32_t i = 0; i < vk.fsr_history_count; i++) {
        const entity_t *previous = &vk.fsr_history[i];
        if (previous->temporal_id != ent->temporal_id ||
            previous->temporal_generation != ent->temporal_generation ||
            previous->model != ent->model || previous->flags != ent->flags)
            continue;
        if (found || Distance(ent->origin, previous->origin) > 128.0f)
            return NULL;
        found = previous;
    }
    return found;
...
{
    vec3_t axis[3];
    const entity_t *previous = vk_fsr_previous_entity(ent);
    if (!previous)
        previous = ent;
    vk_entity_axis(previous, axis);
    vk_entity_mvp_unjittered(out, vk.fsr_previous_fd_valid ?
                            &vk.fsr_previous_fd : &vk.fd, previous, axis);
}
...
static bool vk_create_impact_texture(void)
{
    ...
    for (int y = 0; y < SIZE; y++) {
        for (int x = 0; x < SIZE; x++) {
            ...
            float fy = (y + 0.5f - SIZE * 0.5f) / (SIZE * 0.5f);
            float radius = sqrtf(fx * fx + fy * fy);
            float core_delta = radius / 0.13f;
            float ring_delta = (radius - 0.63f) / 0.075f;
            float core = expf(-0.5f * core_delta * core_delta);
            float ring = expf(-0.5f * ring_delta * ring_delta);
            float edge = Q_clipf((0.98f - radius) / 0.08f, 0.0f, 1.0f);
            float coverage = max(core * 0.92f, ring * edge * 0.88f);
            byte alpha = 255.0f * Q_clipf(coverage, 0.0f, 1.0f);
            pixels[y * SIZE + x] = MakeColor(255, 255, 255, alpha);
        }
    }

    bool uploaded = vk_upload_texture_data(&vk.impact_texture,
                                            SIZE, SIZE, pixels, false);
    Z_Free(pixels);
    if (!uploaded)
        return false;

    vk_update_texture_descriptor_with_sampler(&vk.impact_texture,
                                               vk.postprocess_sampler);
    return true;
}
...
static void vk_world_light_params(const mface_t *face, float color[4], float scroll[4])
{
    ...
    // Preserve a small fractional mode flag for static light faces. The
    // world shaders use it only to seed bloom from the bright texels, keeping
    // the glow attached to the source geometry without a glare billboard.
    if (!(face->drawflags & (SURF_TRANS_MASK | SURF_WARP | SURF_NODRAW)) &&
        (face->texinfo->c.flags & SURF_LIGHT) &&
        !face->texinfo->image->texnum2)
        scroll[2] += 0.125f;
...
#if USE_MD5
static vk_model_t *vk_alias_geometry(vk_model_t *model, const entity_t *ent,
                                    const refdef_t *fd, vk_model_t *view)
{
    if (model->md5_mesh.vertices.buffer && model->md5_mesh.indices.buffer &&
        ...
        *view = *model;
        view->mesh = model->md5_mesh;
        view->alias_line_indices = model->md5_line_indices;
        view->alias_line_index_count = model->md5_line_index_count;
        // Match OpenGL: replacement geometry uses the original model's frame
        // bounds for visibility and projected-shadow sizing.
        view->alias_frames = model->alias_frames;
        view->alias_batches = model->md5_batches;
        view->skins = model->md5_skins;
        view->frame_count = model->md5_frame_count;
        view->alias_batch_count = model->md5_batch_count;
        view->skin_count = model->md5_skin_count;
        view->vertex_count = model->md5_vertex_count;
        return view;
    }
    return model;
}
...
#if USE_MD5
    model = vk_alias_geometry(model, ent, fd, &md5_view);
#endif
...
/* Replay the submitted coverage, including randomized beam geometry, without
 * running the effect simulation a second time. These meshes outlive the frame. */
                                  ...
                                  VkDescriptorSet texture, float alpha)
{
    ...
    vk.fsr_reactive_draws[i].texture = texture;
    vk.fsr_reactive_draws[i].alpha = alpha;
}
...
static void vk_draw_sprite(const entity_t *ent, const refdef_t *fd)
{
    ...
    vk_capture_fsr_reactive(&vk.sprite_quad, mvp, texture->descriptor_set,
                           push.color[3]);
...
static void vk_append_oriented_effect_quad(uint32_t *vertex_count,
                                           const vec3_t origin,
                                           const vec3_t normal, float radius,
                                           const float color[4])
{
    vec3_t right, up, center, corners[4];
    ...
    VectorAdd3(center, right, up, corners[0]);
    VectorSubtract(center, right, corners[1]);
    VectorAdd(corners[1], up, corners[1]);
    VectorAdd(center, right, corners[2]);
    VectorSubtract(corners[2], up, corners[2]);
    VectorSubtract(center, right, corners[3]);
    VectorSubtract(corners[3], up, corners[3]);
...
typedef struct {
    ...
    float distance;
    float seed;
} vk_teleport_vortex_candidate_t;

static uint32_t vk_append_teleport_vortices(const refdef_t *fd,
                                            const vec3_t viewaxis[3],
                                            uint32_t vertex_count)
{
    if (!fd || !vk.raytracing_active || !vk_rt_teleport_vortex ||
        !vk.shockwave_texture.descriptor_set || !fd->entities ||
        fd->num_entities <= 0 || (fd->rdflags & RDF_NOWORLDMODEL) ||
        !vk.world.cache || !vk.world.cache->nodes)
        return vertex_count;

    float strength = Cvar_ClampValue(vk_rt_teleport_vortex, 0.0f, 1.0f);
    uint32_t available = min((VK_MAX_PARTICLE_VERTICES - vertex_count) / 60,
                             (uint32_t)VK_MAX_TELEPORT_VORTICES);
    if (strength <= 0.0f || !available)
        return vertex_count;
    ...
    for (int i = 0; i < fd->num_entities; i++) {
        const entity_t *entity = &fd->entities[i];
        if ((entity->flags & (RF_EFFECT_ONLY | RF_RT_TELEPORT)) !=
            (RF_EFFECT_ONLY | RF_RT_TELEPORT))
            continue;
        if (entity->skinnum < RT_TELEPORT_STANDARD ||
            entity->skinnum > RT_TELEPORT_BOSS)
            continue;
        ...
        candidates[slot].phase = phase;
        candidates[slot].distance = distance;
        candidates[slot].seed = vk_dust_random(vk_dust_hash(
            (int)floorf(entity->origin[0] * 0.0625f),
            (int)floorf(entity->origin[1] * 0.0625f),
            (int)floorf(entity->origin[2] * 0.0625f)));
        if (count < available)
            count++;
    }

    for (uint32_t i = 0; i < count; i++) {
        const vk_teleport_vortex_candidate_t *candidate = &candidates[i];
        const entity_t *entity = candidate->entity;
        float birth = Q_clipf(candidate->phase / 0.12f, 0.0f, 1.0f);
        birth = birth * birth * (3.0f - 2.0f * birth);
        float fade = Q_clipf((1.0f - candidate->phase) / 0.30f,
                             0.0f, 1.0f);
        fade = fade * fade * (3.0f - 2.0f * fade);
        float far_fade = Q_clipf((1536.0f - candidate->distance) / 384.0f,
                                 0.0f, 1.0f);
        float near_fade = Q_clipf(candidate->distance / 48.0f, 0.0f, 1.0f);
        float envelope = birth * fade * far_fade * near_fade;
        float event_scale = Q_clipf(entity->scale, 0.9f, 2.0f);
        float base_color[3] = {
            entity->rgba.u8[0] / 255.0f,
            entity->rgba.u8[1] / 255.0f,
            entity->rgba.u8[2] / 255.0f,
        ...
        if (entity->skinnum == RT_TELEPORT_BOSS) {
            VectorSet(accent_color, 0.72f, 0.28f, 1.0f);
        } else {
            for (int channel = 0; channel < 3; channel++)
                accent_color[channel] = min(base_color[channel] * 0.72f +
                                            0.28f, 1.0f);
        }
        float seed_angle = candidate->seed * (2.0f * M_PIf);

        for (int ring = 0; ring < 2; ring++) {
            float direction = ring ? -1.0f : 1.0f;
            float angle = seed_angle + direction * fd->time *
                (1.8f + ring * 0.65f);
            float tilt = 0.18f + 0.06f * sinf(angle * 1.7f);
            vec3_t normal = {
                cosf(angle) * tilt,
                sinf(angle) * tilt,
                1.0f,
            ...
            origin[2] += event_scale * (8.0f + ring * 18.0f +
                sinf(angle * 1.3f) * 3.0f);
            float radius = event_scale *
                (18.0f + candidate->phase * 32.0f + ring * 7.0f);
            const float *ring_color = ring ? accent_color : base_color;
            float color[4] = {
                ring_color[0], ring_color[1], ring_color[2],
                min((0.18f + strength * 0.28f) *
                    (ring ? 0.78f : 1.0f) * envelope, 0.46f),
            };
            vk_append_oriented_effect_quad(&vertex_count, origin, normal,
                                           radius, color);
        }

        for (int level = 0; level < 4; level++) {
            float travel = fmodf(candidate->phase * 1.65f +
                                 level * 0.25f + candidate->seed, 1.0f);
            float level_fade = sinf(travel * M_PIf);
            vec3_t origin;
            VectorCopy(entity->origin, origin);
            origin[2] += event_scale * (-6.0f + travel * 62.0f);
            float pulse = 0.5f + 0.5f * sinf(
                fd->time * 4.0f + level * 1.7f + seed_angle);
            float radius = event_scale * (5.0f + pulse * 4.0f);
            const float *level_color = (level & 1) ?
                accent_color : base_color;
            float color[4] = {
                level_color[0], level_color[1], level_color[2],
                min((0.10f + strength * 0.18f) * envelope * level_fade,
                    0.28f),
            };
        ...
        for (int mote = 0; mote < 4; mote++) {
            float angle = seed_angle + fd->time *
                (2.4f + mote * 0.13f) + mote * (0.5f * M_PIf);
            float orbit = event_scale *
                (19.0f + 4.0f * sinf(angle * 1.9f + mote));
            float travel = fmodf(candidate->phase * 1.35f +
                                 mote * 0.25f + candidate->seed, 1.0f);
            vec3_t origin = {
                entity->origin[0] + cosf(angle) * orbit,
                entity->origin[1] + sinf(angle) * orbit,
                entity->origin[2] + event_scale * (-3.0f + travel * 58.0f),
            };
            float mote_fade = sinf(travel * M_PIf);
            float color[4] = {
                accent_color[0], accent_color[1], accent_color[2],
                min((0.12f + strength * 0.22f) * envelope * mote_fade,
                    0.34f),
            };
            float radius = event_scale * (2.2f + strength * 1.2f);
            vk_append_effect_quad(&vertex_count, origin, radius, color,
                                  viewaxis);
        }
    }
    return vertex_count;
...
static uint32_t vk_append_energy_collapses(const refdef_t *fd,
                                            const vec3_t viewaxis[3],
                                            uint32_t vertex_count)
{
    if (!fd || !vk.raytracing_active || !vk_rt_energy_collapse ||
        !vk.shockwave_texture.descriptor_set || !fd->entities ||
        fd->num_entities <= 0 || (fd->rdflags & RDF_NOWORLDMODEL) ||
        !vk.world.cache || !vk.world.cache->nodes)
        return vertex_count;

    float strength = Cvar_ClampValue(vk_rt_energy_collapse, 0.0f, 1.0f);
    uint32_t available = min((VK_MAX_PARTICLE_VERTICES - vertex_count) / 48,
                             (uint32_t)VK_MAX_ENERGY_COLLAPSES);
    if (strength <= 0.0f || !available)
        return vertex_count;
    ...
    for (int i = 0; i < fd->num_entities; i++) {
        const entity_t *entity = &fd->entities[i];
        if ((entity->flags & (RF_EFFECT_ONLY | RF_RT_ENERGY_COLLAPSE)) !=
            (RF_EFFECT_ONLY | RF_RT_ENERGY_COLLAPSE))
            continue;
        if (entity->skinnum < RT_ENERGY_BFG_CORE ||
            entity->skinnum > RT_ENERGY_TRACKER)
            continue;
        ...
        candidates[slot].phase = phase;
        candidates[slot].distance = distance;
        candidates[slot].type = entity->skinnum;
        candidates[slot].seed = vk_dust_random(vk_dust_hash(
            (int)floorf(entity->origin[0] * 0.0625f) ^
                (int)entity->angles[1],
            (int)floorf(entity->origin[1] * 0.0625f),
    ...
    for (uint32_t i = 0; i < count; i++) {
        const vk_energy_collapse_candidate_t *candidate = &candidates[i];
        const entity_t *entity = candidate->entity;
        float birth = Q_clipf(candidate->phase / 0.08f, 0.0f, 1.0f);
        birth = birth * birth * (3.0f - 2.0f * birth);
        float fade = Q_clipf((1.0f - candidate->phase) / 0.22f,
                             0.0f, 1.0f);
        fade = fade * fade * (3.0f - 2.0f * fade);
        float far_fade = Q_clipf((1536.0f - candidate->distance) / 384.0f,
                                 0.0f, 1.0f);
        float near_fade = Q_clipf(candidate->distance / 40.0f,
                                  0.0f, 1.0f);
        float envelope = birth * fade * far_fade * near_fade;
        float event_scale = Q_clipf(entity->scale, 0.7f, 1.6f);
        float smooth = candidate->phase * candidate->phase *
            (3.0f - 2.0f * candidate->phase);
        float base_color[3] = {
            entity->rgba.u8[0] / 255.0f,
            entity->rgba.u8[1] / 255.0f,
            entity->rgba.u8[2] / 255.0f,
        ...
        vec3_t rotation = {
            candidate->seed * 180.0f + candidate->phase * 210.0f,
            candidate->seed * 270.0f - candidate->phase * 170.0f,
            candidate->phase * 120.0f,
        };
        vec3_t axes[3];
        AnglesToAxis(rotation, axes);
        float radius = event_scale * (8.0f + (1.0f - smooth) * 50.0f);
        for (int ring = 0; ring < 3; ring++) {
            const float *ring_color = ring == 1 ?
                accent_color : base_color;
            float color[4] = {
                ring_color[0], ring_color[1], ring_color[2],
                min((0.18f + strength * 0.28f) * envelope *
                    (1.0f - ring * 0.10f), 0.46f),
            };
            vk_append_oriented_effect_quad(&vertex_count, entity->origin,
                                           axes[ring],
                                           radius * (0.88f + ring * 0.12f),
                                           color);
        }

        float seed_angle = candidate->seed * (2.0f * M_PIf);
        float orbit = event_scale * (5.0f + (1.0f - smooth) * 58.0f);
        for (int node = 0; node < 4; node++) {
            float angle = seed_angle + node * (0.5f * M_PIf) +
                candidate->phase * (5.8f + node * 0.22f);
            vec3_t origin;
            VectorCopy(entity->origin, origin);
            VectorMA(origin, cosf(angle) * orbit, axes[1], origin);
            VectorMA(origin, sinf(angle) * orbit, axes[2], origin);
            VectorMA(origin, sinf(angle * 1.7f) * orbit * 0.18f,
                     axes[0], origin);
            float color[4] = {
                accent_color[0], accent_color[1], accent_color[2],
                min((0.15f + strength * 0.25f) * envelope, 0.40f),
            };
            float node_radius = event_scale * (3.0f + strength * 1.8f);
            vk_append_effect_quad(&vertex_count, origin, node_radius,
                                  color, viewaxis);
        }

        float final_phase = Q_clipf((candidate->phase - 0.62f) / 0.38f,
                                    0.0f, 1.0f);
        float final_energy = sinf(final_phase * M_PIf);
        float core_color[4] = {
            accent_color[0], accent_color[1], accent_color[2],
            min((0.22f + strength * 0.34f) * final_energy * envelope,
                0.56f),
        };
        float core_radius = event_scale * (6.0f + final_phase * 12.0f);
        vk_append_effect_quad(&vertex_count, entity->origin, core_radius,
                              core_color, viewaxis);
    }
    return vertex_count;
}
...
typedef struct {
    ...
    uint32_t seed;
    int severity;
} vk_landing_dust_candidate_t;

static uint32_t vk_append_landing_dust(const refdef_t *fd,
                                       const vec3_t viewaxis[3],
                                       uint32_t vertex_count)
{
    if (!fd || !vk.raytracing_active || !vk_rt_landing_dust ||
        !vk.particle_texture.descriptor_set || !fd->entities ||
        fd->num_entities <= 0 || (fd->rdflags & RDF_NOWORLDMODEL) ||
        !vk.world.cache || !vk.world.cache->nodes)
        return vertex_count;

    float strength = Cvar_ClampValue(vk_rt_landing_dust, 0.0f, 1.0f);
    uint32_t available = min((VK_MAX_PARTICLE_VERTICES - vertex_count) / 48,
                             (uint32_t)VK_MAX_LANDING_DUST);
    if (strength <= 0.0f || !available)
        return vertex_count;

    const mleaf_t *view_leaf = BSP_PointLeaf(vk.world.cache->nodes,
                                             fd->vieworg);
    if (!view_leaf || (view_leaf->contents[0] & MASK_WATER))
        return vertex_count;

    vk_landing_dust_candidate_t
        candidates[VK_MAX_LANDING_DUST] = { 0 };
    uint32_t count = 0;

    for (int i = 0; i < fd->num_entities; i++) {
        const entity_t *entity = &fd->entities[i];
        if ((entity->flags & (RF_EFFECT_ONLY | RF_RT_LANDING_DUST)) !=
            (RF_EFFECT_ONLY | RF_RT_LANDING_DUST))
            continue;

        int severity = entity->skinnum & RT_LANDING_SEVERITY_MASK;
        if (severity < RT_LANDING_SHORT || severity > RT_LANDING_FAR)
            continue;
        ...
        candidates[slot].phase = phase;
        candidates[slot].distance = distance;
        candidates[slot].severity = severity;
        candidates[slot].seed = vk_dust_hash(
            (int)floorf(entity->origin[0] * 0.125f) ^
                (int)entity->angles[1],
            (int)floorf(entity->origin[1] * 0.125f),
    ...
    static const int mote_counts[] = { 4, 6, 8 };
    for (uint32_t i = 0; i < count; i++) {
        const vk_landing_dust_candidate_t *candidate = &candidates[i];
        const entity_t *entity = candidate->entity;
        float birth = Q_clipf(candidate->phase / 0.10f, 0.0f, 1.0f);
        birth = birth * birth * (3.0f - 2.0f * birth);
        float fade = 1.0f - candidate->phase;
        fade *= sqrtf(max(fade, 0.0f));
        ...
        float near_fade = Q_clipf((candidate->distance - 8.0f) / 24.0f,
                                  0.0f, 1.0f);
        float local_fade = (entity->skinnum & RT_LANDING_LOCAL) ? 0.68f : 1.0f;
        float envelope = birth * fade * far_fade * near_fade * local_fade;
        float event_scale = Q_clipf(entity->scale, 0.65f, 1.35f);
        float travel_phase = candidate->phase * (2.0f - candidate->phase);
        vec3_t tangent, bitangent;
        MakeNormalVectors(candidate->normal, tangent, bitangent);

        for (int mote = 0; mote < mote_counts[candidate->severity]; mote++) {
            uint32_t hash = vk_dust_hash((int)candidate->seed, mote,
                                         candidate->severity);
            float random = vk_dust_random(hash >> 7);
            float angle = vk_dust_random(hash) * (2.0f * M_PIf) +
                candidate->phase * (random - 0.5f) * 0.32f;
            vec3_t direction, origin;
            VectorScale(tangent, cosf(angle), direction);
            VectorMA(direction, sinf(angle), bitangent, direction);
            float travel = event_scale * (13.0f + random * 15.0f) *
                travel_phase;
            VectorMA(entity->origin, travel, direction, origin);
            float arc = event_scale *
                (2.0f + sinf(candidate->phase * M_PIf) *
                 (5.0f + vk_dust_random(hash >> 15) * 7.0f));
            VectorMA(origin, arc, candidate->normal, origin);

            float tint = 0.82f + vk_dust_random(hash >> 20) * 0.18f;
            float color[4] = {
                entity->rgba.u8[0] / 255.0f * tint,
                entity->rgba.u8[1] / 255.0f * tint,
                entity->rgba.u8[2] / 255.0f * tint,
                min((0.14f + strength * 0.27f) * envelope *
                    (0.85f + random * 0.15f), 0.40f),
            };
            float scale = event_scale *
                (2.0f + vk_dust_random(hash >> 11) * 1.35f) *
                (0.72f + candidate->phase * 0.72f);
            if (entity->skinnum & RT_LANDING_LOCAL)
                scale *= 0.88f;
...
static uint32_t vk_append_impact_marks(const refdef_t *fd,
                                       uint32_t vertex_count)
{
    if (!fd || !vk.raytracing_active || !vk_rt_impact_marks ||
        !vk.impact_texture.descriptor_set || !fd->entities ||
        fd->num_entities <= 0 || (fd->rdflags & RDF_NOWORLDMODEL) ||
        !vk.world.cache || !vk.world.cache->nodes)
        return vertex_count;

    float strength = Cvar_ClampValue(vk_rt_impact_marks, 0.0f, 1.0f);
    uint32_t available = min((VK_MAX_PARTICLE_VERTICES - vertex_count) / 6,
                             (uint32_t)VK_MAX_IMPACT_MARKS);
    if (strength <= 0.0f || !available)
        return vertex_count;
    ...
    for (int i = 0; i < fd->num_entities; i++) {
        const entity_t *entity = &fd->entities[i];
        if ((entity->flags & (RF_EFFECT_ONLY | RF_RT_IMPACT)) !=
            (RF_EFFECT_ONLY | RF_RT_IMPACT))
            continue;

        float phase = (entity->oldframe +
            1.0f - Q_clipf(entity->backlerp, 0.0f, 1.0f)) * 0.25f;
        if (phase < 0.0f || phase >= 1.0f)
            continue;

        float distance = Distance(entity->origin, fd->vieworg);
        if (distance < 6.0f || distance >= 768.0f)
            continue;
    ...
    for (uint32_t i = 0; i < count; i++) {
        const vk_impact_candidate_t *candidate = &candidates[i];
        const entity_t *entity = candidate->entity;
        float birth = Q_clipf(candidate->phase / 0.08f, 0.0f, 1.0f);
        birth = birth * birth * (3.0f - 2.0f * birth);
        float fade = 1.0f - candidate->phase;
        ...
        float far_fade = Q_clipf((768.0f - candidate->distance) / 192.0f,
                                 0.0f, 1.0f);
        float near_fade = Q_clipf((candidate->distance - 6.0f) / 18.0f,
                                  0.0f, 1.0f);
        float facing_fade = Q_clipf((candidate->facing - 0.02f) / 0.23f,
                                    0.0f, 1.0f);
        color_t source_color;
        source_color.u32 = entity->skinnum >= 0 ?
            d_8to24table[entity->skinnum & 0xff] : entity->rgba.u32;
        float color[4] = {
            source_color.u8[0] / 255.0f,
            source_color.u8[1] / 255.0f,
            source_color.u8[2] / 255.0f,
            min((0.34f + 0.38f * strength) * birth * fade * far_fade *
                near_fade * facing_fade, 0.72f),
        };
        float event_scale = Q_clipf(entity->scale, 0.65f, 1.25f);
        float radius = (5.0f + candidate->phase * 19.0f) * event_scale *
            (0.90f + strength * 0.15f);
        vk_append_oriented_effect_quad(&vertex_count, entity->origin,
                                       candidate->normal, radius, color);
    }
...
static void vk_draw_particles(const refdef_t *fd)
{
    ...
    vertex_count = vk_append_landing_dust(fd, viewaxis, vertex_count);
    ...
    vk.particle_motion_offset = offset;
    vk.particle_motion_ranges[0] = 0;
    vk.particle_motion_ranges[1] = shockwave_first_vertex;
    vk.particle_motion_ranges[2] = impact_first_vertex;
    vk.particle_motion_ranges[3] = electric_first_vertex;
    vk.particle_motion_ranges[4] = vertex_count;
    ...
    if (impact_first_vertex > shockwave_first_vertex) {
        vk_bind_pipeline(cmd, VK_PIPELINE_BIND_POINT_GRAPHICS,
                         vk.particle_add_pipeline ?
                            vk.particle_add_pipeline : vk.sprite_pipeline);
        vk_bind_texture_descriptor(cmd, vk.shockwave_texture.descriptor_set);
        vk.CmdDraw(cmd, impact_first_vertex - shockwave_first_vertex, 1,
                                 ...
                                 float width)
{
    ...
    vk_push_constants(cmd, sizeof(push), &push);
    vk.CmdDrawIndexed(cmd, vk.sprite_quad.index_count, 1, 0, 0, 0);
    vk_capture_fsr_reactive(&vk.sprite_quad, mvp, vk.beam_texture.descriptor_set,
                           color[3]);
    c.trisDrawn += vk.sprite_quad.index_count / 3;
    vk_count_batch3d();
    vk_draw_fx_outlines(&vk.sprite_quad, &vk.sprite_quad_line_indices,
                                      ...
                                      float width)
{
    ...
    vk_capture_fsr_reactive(&vk.beam_cylinder, mvp, vk.textures[0].descriptor_set,
                           color[3]);
...
static void vk_log_perf_stats(void)
{
    ...
    if (vk.separate_presentation) {
        ...
        Com_Printf("VK FSR3 perf: auto=%s phase=%d quality=%s motion=%s native=%lluus fsr=%lluus samples=%u warmup=%u timing=%s mipbias=%.2f\n",
                   vk_fsr_auto_enabled() ? "yes" : "no",
                   vk.fsr_auto_phase,
                   vk_fsr_quality_name(vk.fsr_auto_quality),
                   vk.fsr_motion_fast ? "fast" :
                       (r_fsr_motion && !Q_stricmp(r_fsr_motion->string, "full") ? "full" : "auto"),
                   (unsigned long long)vk.fsr_auto_native_usec,
                   (unsigned long long)vk.fsr_auto_fsr_usec,
...
bool VKR_Init(bool total)
{
    ...
    if (!vk_create_shockwave_texture())
        Com_WPrintf("Couldn't create Vulkan shockwave texture: %s\n",
                    Com_GetLastError());
    if (!vk_create_impact_texture())
        Com_WPrintf("Couldn't create Vulkan impact texture: %s\n",
                    Com_GetLastError());
...
void VKR_RenderFrame(const refdef_t *fd)
{
    ...
    if (vk.frame_fsr && vk.fsr3) {
        ...
        if (vk.fsr_pending_history_count)
            memcpy(vk.fsr_pending_history, fd->entities,
                   vk.fsr_pending_history_count * sizeof(entity_t));
...
static void vk_fsr_motion_parameters(vk_fsr_motion_push_t *push, const refdef_t *fd)
{
    float sx = (float)vk.render_extent.width / vk.swapchain_extent.width;
    float sy = (float)vk.render_extent.height / vk.swapchain_extent.height;
    int x = Q_clip(Q_rint(fd->x * sx), 0, (int)vk.render_extent.width);
    int y = Q_clip(Q_rint(fd->y * sy), 0, (int)vk.render_extent.height);
    int x2 = Q_clip(Q_rint((fd->x + max(fd->width, 1)) * sx), x, (int)vk.render_extent.width);
    int y2 = Q_clip(Q_rint((fd->y + max(fd->height, 1)) * sy), y, (int)vk.render_extent.height);
    Vector4Set(push->viewport, x, y, max(x2 - x, 1), max(y2 - y, 1));
    push->motion_scale[0] = 0.5f * push->viewport[2] / vk.render_extent.width;
    push->motion_scale[1] = 0.5f * push->viewport[3] / vk.render_extent.height;
    push->jitter[0] = -2.0f * vk.fsr_jitter[0] / vk.render_extent.width;
    push->jitter[1] = 2.0f * vk.fsr_jitter[1] / vk.render_extent.height;
    push->alpha = 1.0f;
}

/* Static geometry normally needs only reactive coverage. Full mode is retained
 * as a geometry reference; moving inline models always override camera motion. */
static void vk_draw_fsr_motion_world(const refdef_t *fd, const entity_t *ent, bool opaque)
{
    if (!vk.world.cache || !vk.world.mesh.index_count)
        return;
    const mmodel_t *model = NULL;
    vk_fsr_motion_push_t push = { 0 };
    vk_fsr_motion_parameters(&push, fd);
    if (ent) {
        int index = ~ent->model;
        if (index < 1 || index >= vk.world.cache->nummodels)
            return;
        model = &vk.world.cache->models[index];
        vec3_t axis[3];
        vk_entity_axis(ent, axis);
        if (vk_bmodel_culled(model, ent, axis))
            return;
        vk_entity_mvp_unjittered(push.mvp, fd, ent, axis);
        vk_previous_entity_mvp(push.previous_mvp, ent);
    } else {
        vk_viewproj_matrix(push.mvp, fd, false);
        memcpy(push.previous_mvp, vk.fsr_previous_viewproj_valid && !vk.fsr_reset ?
               vk.fsr_previous_viewproj : push.mvp, sizeof(mat4_t));
    }
    VkCommandBuffer cmd = vk.command_buffers[vk.current_image];
    VkDeviceSize offset = 0;
    vk.CmdBindVertexBuffers(cmd, 0, 1, &vk.world.mesh.vertices.buffer, &offset);
    vk.CmdBindIndexBuffer(cmd, vk.world.mesh.indices.buffer, 0, VK_INDEX_TYPE_UINT32);
    for (uint32_t i = 0; i < vk.world.face_count; i++) {
        const vk_world_face_t *draw = &vk.world.faces[i];
        mface_t *face = draw->face;
        if (!face || !face->texinfo || !draw->index_count)
            continue;
        if (face->drawflags & (SURF_SKY | SURF_NODRAW))
            continue;
        if (model) {
            if (face < model->firstface || face >= model->firstface + model->numfaces)
                continue;
        } else if (face->drawframe != vk.world.drawframe ||
                   vk_world_face_backfacing(face, fd->vieworg)) {
            continue;
        }
        bool translucent = !vk_world_face_in_pass(face, VK_WORLD_OPAQUE) ||
            (ent && (ent->flags & RF_TRANSLUCENT));
        if (!opaque && !translucent)
            continue;
        if (translucent && !vk.physical_device_features.independentBlend)
            continue;
        const image_t *image = vk_world_face_image(face, fd, ent);
        const vk_texture_t *texture = image ? vk_texture_for_index(image->texnum, true) : NULL;
        if (!texture)
            continue;
        push.alpha = ent && (ent->flags & RF_TRANSLUCENT) ? ent->alpha :
        ...
        vk.CmdBindDescriptorSets(cmd, VK_PIPELINE_BIND_POINT_GRAPHICS,
                                 vk.fsr_motion_pipeline_layout, 0, 1,
...
static void vk_draw_fsr_motion_aliases(const refdef_t *fd)
{
    if (!fd || !vk.fsr_motion_alias_pipeline ||
        (vk_drawentities && !vk_drawentities->integer) ||
        !fd->entities || fd->num_entities <= 0)
        return;
    ...
    for (int i = 0; i < fd->num_entities; i++) {
        ...
        if (!ent->model || (ent->model & BIT(31)))
            continue;
        if ((ent->flags & (RF_EFFECT_ONLY | RF_BEAM | RF_FLARE | RF_BLOOM_ONLY)) ||
            !R_EntityVisibleAcrossLiquids(vk.world.cache, fd, ent))
            continue;
        ...
        const entity_t *previous = vk_fsr_previous_entity(ent);
#if USE_MD5
        vk_model_t current_view, previous_view;
        vk_model_t *base = model;
        model = vk_alias_geometry(base, ent, fd, &current_view);
        if (previous && vk_alias_geometry(base, previous, &vk.fsr_previous_fd,
                                         &previous_view)->mesh.vertices.buffer !=
                        model->mesh.vertices.buffer)
            previous = NULL;
#endif
        vk_alias_lerp_t lerp = vk_alias_lerp_for_entity(model, ent, fd);
        vk_alias_lerp_t previous_lerp = previous ?
            vk_alias_lerp_for_entity(model, previous, &vk.fsr_previous_fd) : lerp;
        VkDeviceSize offsets[4] = {
            (VkDeviceSize)lerp.frame * model->vertex_count * sizeof(vk_vertex_t),
            (VkDeviceSize)lerp.oldframe * model->vertex_count * sizeof(vk_vertex_t),
            (VkDeviceSize)previous_lerp.frame * model->vertex_count * sizeof(vk_vertex_t),
            (VkDeviceSize)previous_lerp.oldframe * model->vertex_count * sizeof(vk_vertex_t),
        };
        VkBuffer buffers[4] = {
            model->mesh.vertices.buffer,
            model->mesh.vertices.buffer,
        ...
        vk.CmdBindPipeline(cmd, VK_PIPELINE_BIND_POINT_GRAPHICS,
                           ...
                           vk.fsr_motion_alias_reactive_pipeline : vk.fsr_motion_alias_pipeline);
        ...
        for (uint32_t batch = 0; batch < model->alias_batch_count; batch++) {
            ...
            const image_t *skin = vk_skin_for_alias_batch(model, draw, ent);
            if (!skin)
                continue;
            const vk_texture_t *texture = vk_texture_for_index(skin->texnum, true);
            if (!texture)
                continue;
            push.viewport[0] = (skin->flags & IF_TRANSPARENT) ? 0.666f : 0.0f;
            vk.CmdPushConstants(cmd, vk.fsr_motion_pipeline_layout,
                                VK_SHADER_STAGE_VERTEX_BIT | VK_SHADER_STAGE_FRAGMENT_BIT,
                                0, sizeof(push), &push);
            vk.CmdBindDescriptorSets(cmd, VK_PIPELINE_BIND_POINT_GRAPHICS,
...
static void vk_draw_fsr_motion_particles(const refdef_t *fd)
{
    ...
    vk.CmdPushConstants(cmd, vk.fsr_motion_pipeline_layout,
                        VK_SHADER_STAGE_VERTEX_BIT | VK_SHADER_STAGE_FRAGMENT_BIT,
                        0, sizeof(push), &push);
    const vk_texture_t *textures[] = { &vk.particle_texture,
        &vk.shockwave_texture, &vk.impact_texture, &vk.beam_texture };
    for (unsigned i = 0; i < q_countof(textures); i++) {
        uint32_t first = vk.particle_motion_ranges[i];
        uint32_t count = vk.particle_motion_ranges[i + 1] - first;
        if (!count)
            continue;
        vk.CmdBindDescriptorSets(cmd, VK_PIPELINE_BIND_POINT_GRAPHICS,
                                vk.fsr_motion_pipeline_layout, 0, 1,
                                &textures[i]->descriptor_set, 0, NULL);
        vk.CmdDraw(cmd, count, 1, first, 0);
    }
}

static void vk_draw_fsr_reactive(void)
{
    VkCommandBuffer cmd = vk.command_buffers[vk.current_image];
    VkDeviceSize offset = 0;
    vk.CmdBindPipeline(cmd, VK_PIPELINE_BIND_POINT_GRAPHICS,
                       vk.fsr_motion_world_reactive_pipeline);
    for (unsigned i = 0; i < vk.fsr_reactive_draw_count; i++) {
        const vk_mesh_t *mesh = vk.fsr_reactive_draws[i].mesh;
        vk_fsr_motion_push_t push = { 0 };
        memcpy(push.mvp, vk.fsr_reactive_draws[i].mvp, sizeof(mat4_t));
        ...
        vk.CmdBindDescriptorSets(cmd, VK_PIPELINE_BIND_POINT_GRAPHICS,
                                vk.fsr_motion_pipeline_layout, 0, 1,
                                &vk.fsr_reactive_draws[i].texture, 0, NULL);
        vk.CmdPushConstants(cmd, vk.fsr_motion_pipeline_layout,
                           VK_SHADER_STAGE_VERTEX_BIT | VK_SHADER_STAGE_FRAGMENT_BIT,
                           0, sizeof(push), &push);
...
static void vk_render_fsr_motion(const refdef_t *fd)
{
    ...
    vk_write_fsr_timestamp(VK_TIMESTAMP_MOTION_BEGIN);
    VkCommandBuffer cmd = vk.command_buffers[vk.current_image];
    if (vk.render_pass_active) {
        vk.CmdEndRenderPass(cmd);
        vk.render_pass_active = false;
    ...
    vk_fsr_motion_push_t camera = { 0 };
    vk_viewproj_matrix(camera.mvp, fd, false);
    memcpy(camera.previous_mvp, vk.fsr_previous_viewproj_valid && !vk.fsr_reset ?
           vk.fsr_previous_viewproj : camera.mvp, sizeof(mat4_t));
    vk_fsr_motion_parameters(&camera, fd);
    vk.CmdBindPipeline(cmd, VK_PIPELINE_BIND_POINT_GRAPHICS, vk.fsr_motion_camera_pipeline);
    vk.CmdBindDescriptorSets(cmd, VK_PIPELINE_BIND_POINT_GRAPHICS,
                             vk.fsr_motion_pipeline_layout, 0, 1,
                             &vk.fsr_depth_descriptor, 0, NULL);
    vk.CmdPushConstants(cmd, vk.fsr_motion_pipeline_layout,
                        VK_SHADER_STAGE_VERTEX_BIT | VK_SHADER_STAGE_FRAGMENT_BIT,
                        0, sizeof(camera), &camera);
    vk.CmdDraw(cmd, 3, 1, 0, 0);
    if (!vk.fsr_motion_fast) {
        if (!(fd->rdflags & RDF_NOWORLDMODEL) &&
            (!vk_drawworld || vk_drawworld->integer))
            vk_draw_fsr_motion_world(fd, NULL,
                r_fsr_motion && !Q_stricmp(r_fsr_motion->string, "full"));
        for (int i = 0; i < fd->num_entities; i++)
            if ((!vk_drawentities || vk_drawentities->integer) &&
                (fd->entities[i].model & BIT(31)) &&
                !(fd->entities[i].flags & (RF_EFFECT_ONLY | RF_BLOOM_ONLY)) &&
                R_EntityVisibleAcrossLiquids(vk.world.cache, fd, &fd->entities[i]))
                vk_draw_fsr_motion_world(fd, &fd->entities[i], true);
        vk_draw_fsr_motion_aliases(fd);
        if (vk.physical_device_features.independentBlend) {
            vk_draw_fsr_motion_particles(fd);
            vk_draw_fsr_reactive();
        }
    }
...
static void vk_clear_fsr_motion_targets(VkCommandBuffer cmd, bool clear_reactive)
{
    VkImageSubresourceRange range = {
        .aspectMask = VK_IMAGE_ASPECT_COLOR_BIT,
        .levelCount = 1,
    ...
    vk_transition_color_target(cmd, &vk.fsr_motion_texture,
                               &vk.fsr_motion_layout,
                               VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL,
                               VK_ACCESS_TRANSFER_WRITE_BIT,
                               VK_PIPELINE_STAGE_TRANSFER_BIT);
    vk.CmdClearColorImage(cmd, vk.fsr_motion_texture.image,
                          VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL,
                          &zero, 1, &range);
    vk_transition_color_target(cmd, &vk.fsr_motion_texture,
                               &vk.fsr_motion_layout,
...
static bool vk_prepare_direct_pause_cache(VkCommandBuffer cmd)
{
    if (!vk.fsr_swapchain_direct || !vk.fsr_pause_texture.image ||
        !vk.fsr_direct_source_valid ||
        vk.fsr_direct_source_image >= vk.swapchain_image_count)
        return false;

    uint32_t source_image = vk.fsr_direct_source_image;
    if (source_image != vk.current_image && vk.image_fences[source_image]) {
        uint64_t start = vk_time_usec();
        VkResult result = vk.WaitForFences(vk.device, 1,
                                           &vk.image_fences[source_image],
                                           VK_TRUE, UINT64_MAX);
        vk.wait_usec += vk_time_usec() - start;
        if (result != VK_SUCCESS) {
            vk_handle_device_lost("vkWaitForFences", result);
            Com_EPrintf("vkWaitForFences failed while caching paused FSR3 output: Vulkan error %d\n",
                        result);
            return false;
        }
    }

    VkImageLayout source_restore_layout = source_image == vk.current_image ?
        VK_IMAGE_LAYOUT_COLOR_ATTACHMENT_OPTIMAL : VK_IMAGE_LAYOUT_PRESENT_SRC_KHR;
    vk_transition_image(cmd, source_image,
                        VK_IMAGE_LAYOUT_TRANSFER_SRC_OPTIMAL,
                        VK_ACCESS_TRANSFER_READ_BIT,
                        VK_PIPELINE_STAGE_TRANSFER_BIT);
    vk_transition_color_target(cmd, &vk.fsr_pause_texture,
                               &vk.fsr_pause_layout,
                               VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL,
                               VK_ACCESS_TRANSFER_WRITE_BIT,
                               VK_PIPELINE_STAGE_TRANSFER_BIT);
    ...
    vk.CmdCopyImage(cmd, vk.swapchain_images[source_image],
                    VK_IMAGE_LAYOUT_TRANSFER_SRC_OPTIMAL,
                    vk.fsr_pause_texture.image,
                    VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL,
                    1, &region);
    vk_transition_color_target(cmd, &vk.fsr_pause_texture,
                               &vk.fsr_pause_layout,
                               ...
                               VK_ACCESS_SHADER_READ_BIT,
                               VK_PIPELINE_STAGE_FRAGMENT_SHADER_BIT);
    vk_transition_image(cmd, source_image, source_restore_layout,
                        source_restore_layout == VK_IMAGE_LAYOUT_PRESENT_SRC_KHR ? 0 :
                        VK_ACCESS_COLOR_ATTACHMENT_WRITE_BIT,
                        source_restore_layout == VK_IMAGE_LAYOUT_PRESENT_SRC_KHR ?
                        VK_PIPELINE_STAGE_BOTTOM_OF_PIPE_BIT :
                        VK_PIPELINE_STAGE_COLOR_ATTACHMENT_OUTPUT_BIT);
...
static bool vk_dispatch_fsr(void)
{
    vk_texture_t *input_texture = vk.fsr_scene_direct ?
        &vk.scene_texture : &vk.fsr_input_texture;
    VkImageLayout *input_layout = vk.fsr_scene_direct ?
        &vk.scene_layout : &vk.fsr_input_layout;
    bool reactive_required = !vk.fsr_motion_fast;
    bool direct_output = vk.fsr_swapchain_direct &&
        !vk_fsr_frame_generation_requested();
    VkImage output_image = direct_output ? vk.swapchain_images[vk.current_image] :
        vk.fsr_output_texture.image;
    VkImageView output_view = direct_output ? VK_NULL_HANDLE :
        vk.fsr_output_texture.view;
    uint32_t output_width = direct_output ? vk.swapchain_extent.width :
        vk.fsr_output_texture.width;
    uint32_t output_height = direct_output ? vk.swapchain_extent.height :
        vk.fsr_output_texture.height;

    if (!vk.frame_fsr)
        return false;
    if (!vk.fsr3 || (!direct_output && (!output_image || !output_view)) ||
        !input_texture->image ||
        !input_texture->view || !vk.depth_image ||
        !vk.depth_sample_view || !vk.fsr_motion_texture.image ||
        !vk.fsr_motion_texture.view ||
        (reactive_required && (!vk.fsr_reactive_texture.image ||
                               !vk.fsr_reactive_texture.view)) ||
        vk.fsr_output_format == VK_FORMAT_UNDEFINED ||
        input_texture->width != vk.render_extent.width ||
    ...
    vk_image_barrier_batch_t pre_fsr_barriers = { .cmd = cmd };
    ...
    bool dispatched = Q2_FSR3_Dispatch(
        ...
        vk.fsr_output_format,
        vk.fsr_jitter[0], vk.fsr_jitter[1],
        r_fsr_sharpness ? Cvar_ClampValue(r_fsr_sharpness, 0.0f, 1.0f) : 0.0f,
        frame_time_ms, fov_y,
        vk_znear ? Cvar_ClampValue(vk_znear, 0.1f, 4095.0f) : 2.0f,
        max(vk_projection_zfar(vk.fd_valid ? vk.fd.rdflags : 0),
            (vk_znear ? Cvar_ClampValue(vk_znear, 0.1f, 4095.0f) : 2.0f) + 1.0f),
    ...
    if (!dispatched) {
        ...
        vk_composite_presentation_texture(input_texture);
    ...
    if (!frame_reset && frame_generation_prepared &&
        ...
        vk_write_fsr_timestamp(VK_TIMESTAMP_FRAMEGEN_BEGIN);
...
static void vk_finish_postprocess_scene(void)
{
    ...
    vk.fsr_scene_direct = vk.frame_fsr && !fsr_input_active &&
        !bloom && !waterwarp && !vk.frame_ssr;
    ...
    if (vk.separate_presentation && !vk.fsr_composited) {
        ...
        vk_begin_presentation(false);
        vk_write_fsr_timestamp(VK_TIMESTAMP_PRESENT_BEGIN);
        vk_composite_presentation_texture(vk.fsr_scene_direct ?
                                          &vk.scene_texture :
                                          &vk.fsr_input_texture);
        vk_write_fsr_timestamp(VK_TIMESTAMP_PRESENT_END);
        vk.fsr_composited = true;
        vk.fsr_reset = true;
    }
...
void VKR_BeginFrame(void)
{
    ...
    float bias = 0.0f;
    if (vk.separate_presentation && vk.render_extent.width && vk.swapchain_extent.width) {
        bias = r_fsr_mip_bias && Q_stricmp(r_fsr_mip_bias->string, "auto") ?
            r_fsr_mip_bias->value :
            log2f((float)vk.render_extent.width / vk.swapchain_extent.width) - 1.0f;
        if (!isfinite(bias))
            bias = 0.0f;
        float limit = vk.physical_device_properties.limits.maxSamplerLodBias;
        bias = Q_clipf(bias, -limit, limit);
    }
    if (vk.fsr_sampler_bias != bias || !vk.fsr_sampler) {
        vk.fsr_sampler_bias = bias;
        vk_texturemode_changed(NULL);
    }
    ...
    if (vk.timestamp_query_pool && vk.timestamp_valid[vk.frame_index]) {
        ...
        if (vk_read_timestamp_pair(first + VK_TIMESTAMP_FRAME_BEGIN,
            ...
            if ((mask & (1u << VK_TIMESTAMP_MOTION_BEGIN)) &&
                ...
                vk_read_timestamp_pair(first + VK_TIMESTAMP_MOTION_BEGIN,
                                       &vk.fsr_gpu_motion_usec);
            vk.fsr_gpu_upscale_usec = 0;
            if ((mask & (1u << VK_TIMESTAMP_FSR_BEGIN)) &&
                (mask & (1u << VK_TIMESTAMP_FSR_END)))
                vk_read_timestamp_pair(first + VK_TIMESTAMP_FSR_BEGIN,
                                       &vk.fsr_gpu_upscale_usec);
            vk.fsr_gpu_frame_generation_usec = 0;
            if ((mask & (1u << VK_TIMESTAMP_FRAMEGEN_BEGIN)) &&
                (mask & (1u << VK_TIMESTAMP_FRAMEGEN_END)))
                vk_read_timestamp_pair(first + VK_TIMESTAMP_FRAMEGEN_BEGIN,
                                       &vk.fsr_gpu_frame_generation_usec);
            vk.fsr_gpu_presentation_usec = 0;
            if ((mask & (1u << VK_TIMESTAMP_PRESENT_BEGIN)) &&
                (mask & (1u << VK_TIMESTAMP_PRESENT_END)))
                vk_read_timestamp_pair(first + VK_TIMESTAMP_PRESENT_BEGIN,
                                       &vk.fsr_gpu_presentation_usec);
        }
        vk.timestamp_valid[vk.frame_index] = false;
        vk.timestamp_scope_mask[vk.frame_index] = 0;
    }
    ...
    if (vk.timestamp_query_pool) {
        uint32_t first = vk.frame_index * VK_TIMESTAMP_QUERY_COUNT;
        vk.CmdResetQueryPool(cmd, vk.timestamp_query_pool, first,
                             VK_TIMESTAMP_QUERY_COUNT);
        vk.CmdWriteTimestamp(cmd, VK_PIPELINE_STAGE_TOP_OF_PIPE_BIT,
                             vk.timestamp_query_pool,
                             first + VK_TIMESTAMP_FRAME_BEGIN);
    }
    ...
    if (vk.separate_presentation) {
        ...
        if (paused && vk.fsr_output_valid &&
            (!vk.fsr_swapchain_direct || vk.fsr_pause_cache_valid)) {
            ...
            vk.frame_fsr = false;
            vk_composite_presentation_texture(vk.fsr_swapchain_direct ?
                                              &vk.fsr_pause_texture :
    ...
    VkRenderPassBeginInfo render_pass_info = {
        ...
        .renderArea = {
            .offset = { 0, 0 },
            .extent = postprocess ? vk.render_extent : vk.swapchain_extent,
        },
        .clearValueCount = clear_count,
        .pClearValues = clear,
    };
...
void VKR_EndFrame(void)
{
    ...
    if (vk.timestamp_query_pool) {
        vk.CmdWriteTimestamp(cmd, VK_PIPELINE_STAGE_BOTTOM_OF_PIPE_BIT,
                             vk.timestamp_query_pool,
                             vk.frame_index * VK_TIMESTAMP_QUERY_COUNT +
                             VK_TIMESTAMP_FRAME_END);
    }
    ...
    if (vk.fsr_presentation_copy)
        wait_stage |= VK_PIPELINE_STAGE_TRANSFER_BIT;
    if (vk.fsr_swapchain_direct && (vk.frame_fsr || vk.fsr_pause_reuse))
    ...
    if (vk.frame_fsr && vk.fsr_output_valid && !vk.fsr_pause_reuse &&
        !vk_fsr_scene_paused()) {
        vk.fsr_previous_fd = vk.fd;
        vk.fsr_previous_fd.entities = NULL;
        vk.fsr_previous_fd_valid = true;
        vk_viewproj_matrix(vk.fsr_previous_viewproj, &vk.fd, false);
        vk.fsr_previous_viewproj_valid = true;
        vk.fsr_history_count = vk.fsr_pending_history_count;
        memcpy(vk.fsr_history, vk.fsr_pending_history,
               vk.fsr_history_count * sizeof(entity_t));
    }

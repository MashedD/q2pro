...
r_fsr_auto::
    ...
    renderer measures a native-resolution baseline, tests progressively lower
    quality profiles, and keeps the profile with the lowest measured frame
    cost when it is at least 5% faster than native rendering. If no profile
    improves frame time, FSR is disabled for the current session. The selected
    r_fsr_quality value is the highest quality profile that automatic mode may
    test. Set this to 0 to keep r_fsr_quality fixed. Default value is 1.

r_fsr_motion::
    Selects the FSR motion-vector path: ‘full’ renders the additional scene
    motion pass for best temporal quality, ‘fast’ clears motion/reactive data
    and skips that pass, and ‘auto’ selects the fast path for ordinary
    upscaling. Frame generation automatically requires full motion vectors.
    The fast path can show more ghosting during movement. Default value is
    ‘fast’.

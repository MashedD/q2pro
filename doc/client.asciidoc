...
r_fsr_auto::
    Enables the Vulkan FSR3 performance guard. When enabled, the renderer
    measures a native-resolution warm-up and an FSR trial, keeps FSR only when
    its measured frame cost is not more than 5% slower, and otherwise falls
    back for the current session. Default value is 1.

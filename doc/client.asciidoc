...
vk_rt_reflections::
    ...
    walls, clamped between 0 and 1. Eight depth-march steps keep the effect
    affordable; objects outside the screen or hidden behind other geometry
    cannot be reflected. Requires multisampling 1x. Default value is 0.35;
    value 0 disables it.

materials/rt_reflections.mat::
    Optional exact-name reflection allowlist used by the Vulkan SSR pass.
    When present, unlisted textures do not reflect. Entries use a Q2RTX-style
    block with `reflect` and `roughness` values clamped between 0 and 1:
    ...
    `materials/baseq2.mat` are not parsed for SSR.

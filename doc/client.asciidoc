...
vk_rt_muzzle_plumes::
    Controls brief directional fire and energy plumes at player and monster
    muzzles while Vulkan ray tracing is active, clamped between 0 and 1.
    Ballistic, explosive, rail, beam, blaster, BFG, disruptor, and expansion
    weapons retain distinct colors. Each plume combines a tapered two-segment
    core with two short outward filaments and lasts only for the existing 50 ms
    muzzle-flash interval. First-person plumes use reduced length, width, and
    brightness to avoid obscuring the view; other players and monsters follow
    BSP visibility, area, distance, and liquid-medium restrictions. Weapons
    without a muzzle-flash model receive an effect-only marker, while existing
    flash models, skins, lights, sounds, and animations remain unchanged. The
    nearest 12 active plumes reuse the beam texture, additive pipeline, mapped
    particle buffer, fog, and shared beam batch, adding at most 96 triangles
    and one draw while active with no new shader, texture, descriptor, render
    pass, ray query, persistent marker, or per-frame allocation. The effect
    also respects `cl_muzzleflashes`. Changes apply immediately. Default value
    is 0.75; value 0 disables the effect.
...
vk_rt_projectile_wakes::
    Controls compact energy rings behind moving projectiles while Vulkan ray
    tracing is active, clamped between 0 and 1. Rockets, grenades, blaster and
    hyperblaster bolts, ion and plasma bolts, tracker projectiles, and BFG balls
    use distinct colors and sizes. Two clearly visible rings follow each
    projectile perpendicular to its interpolated travel direction, with a
    restrained animated pulse. Existing projectile models, particles, trails,
    lights, sounds, and gameplay remain unchanged. Initial, stationary, and
    teleport-sized movements are rejected, while near-camera and distance fades,
    BSP visibility, area, and liquid-medium restrictions prevent distracting or
    misplaced rings. The nearest eight projectiles reuse the procedural
    shockwave texture, additive pipeline, mapped particle buffer, fog, and
    shared shockwave batch, adding at most 32 triangles and one draw while active
    with no new shader, texture, descriptor, render pass, ray query, persistent
    history, or per-frame allocation. Changes apply immediately. Default value
    is 0.75; value 0 disables the effect.

vk_rt_light_trails::
    Controls short colored trails behind strong moving dynamic lights while
    Vulkan ray tracing is active, clamped between 0 and 1. Lights with at
    least 160 intensity reuse the existing eight-entry transient-light tracker
    and retain one smoothly lagged segment. Movement below 6 units is rejected,
    teleport-like changes are reset, visible length is capped at 96 units, and
    the segment fades for 0.10 seconds after its source disappears. Trails use
    the existing depth-tested beam geometry and add at most eight normal draws;
    bloom adds at most eight matching lower-energy draws when enabled. No new
    shaders, ray queries, texture samples, or render passes are required.
    Changes apply immediately and remain independent of `vk_rt_afterglow`.
    Default value is 0.55; value 0 disables the effect.
...
gl_glare::
    Enables lens glare sprites on bright glowmapped world surfaces. Visibility
    is checked against the rendered scene and fades smoothly using
    ‘gl_flarespeed’. Default value is 0.

gl_glare_threshold::
    Minimum lightmap brightness required for a glowmapped surface to emit glare.
    Higher values reduce the number of glare sources. Default value is 0.3.

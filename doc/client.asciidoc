...
vk_rt_weapon_glow::
    Controls a slow colored contour glow on the local first-person energy
    weapon while Vulkan ray tracing is active, clamped between 0 and 1. The
    base-game Blaster uses warm amber, HyperBlaster cyan-blue, Railgun violet,
    and BFG mint-green; conventional, custom, and unrecognized weapons remain
    unchanged. The overlay uses the weapon's depth-hacked projection, so it
    does not produce a world-space light or affect other players or monsters.
    It reuses the existing white shell texture and additive alias pipelines,
    adds one draw per viewmodel batch, and contributes a reduced matching pass
    to existing bloom when enabled. No shader, texture, descriptor, render
    pass, ray query, protocol field, or persistent allocation is required.
    Changes apply immediately. Default value is 0.65; value 0 disables the
    effect.
...
Bloom remains disabled by default and adds no work while ‘gl_bloom’ is 0.
...
gl_bloom::
    ...
    The setting is archived. Default value is 0 (disabled). Higher values
...
vk_bloom_streaks::
    Controls horizontal anamorphic streaks from Vulkan bloom sources, clamped
    between 0 and 1. The effect stretches the complete horizontal Gaussian
    radius from 1x to 4x without adding samples or passes, while preserving
    normalized bloom energy. Value 0 restores ordinary Gaussian bloom. Default
    value is 0.45 (2.35x); the setting has no effect while ‘gl_bloom’ is 0.

vk_bloom_ghosts::
    Controls Vulkan bloom lens ghosts and their soft halo, clamped between 0
    and 1. Three brightness-gated images of bloom sources are distributed
    across the screen center; the strongest has mild red/blue edge separation.
    Source energy and screen edges are capped so bright lamps do not wash out
    the frame or form clamped bars. The effect runs in the existing
    low-resolution bloom downscale and adds no images, descriptors, draws, or
    passes. It works with raster and ray-traced bloom sources. Default value is
    0.5; value 0 restores the previous bloom exactly. The setting has no effect
    while ‘gl_bloom’ is 0.

vk_bloom_shafts::
    Controls radial light shafts from bright Vulkan bloom sources, clamped
    between 0 and 1. Four brightness-gated samples extend each source toward
    the optical center in the existing low-resolution bloom downscale pass.
    Center and screen-edge fades plus a separate luminance cap prevent broad
    scene washout. The effect adds no images, descriptors, draws, render
    passes, or ray queries, and works with raster and ray-traced bloom sources.
    Default value is 0.4; value 0 skips the extra samples and restores the
    previous bloom exactly. The setting has no effect while ‘gl_bloom’ is 0.
...
gl_glare::
    Enables lens glare sprites on bright glowmapped world surfaces and opaque
    static BSP faces marked `SURF_LIGHT`. The latter provide a lightmap-tinted
    fallback when a glowmap asset is unavailable, so the effect works in
    OpenGL and raster Vulkan with `r_glowmaps` disabled. Visibility is checked
    against the rendered scene and remains within the viewer's water, slime,
    lava, or air medium to avoid leaking through translucent liquid surfaces.
    It fades smoothly using ‘gl_flarespeed’; RT-only sources fade out when
    `vk_rt_emissive` is 0. Default value is 0.

gl_glare_threshold::
    Minimum lightmap brightness required for a glowmapped surface to emit glare.
    Static `SURF_LIGHT` fallback sources use their normalized map light value
    instead. Higher values reduce the number of glare sources. Default value
    is 0.3.

gl_glare_size::
    Base size of surface glare sprites. Glare size also scales with source
    brightness and distance from the viewer. Default value is 24.

gl_glare_intensity::
    Intensity factor for surface glare sprites. Default value is 0.5.

...
vk_raytracing::
    ...
    Stable world AO is baked once when a map loads into an atlas capped at
    2048x2048. Its adaptive one-to-four samples are bounded to approximately
    four million rays per map, so successful bakes add no per-frame AO rays.
    If the bake is unavailable, the renderer falls back to the previous
    distance-limited single-ray AO path. Dynamic and emissive shadow opacity
    uses blocker distance, keeping contact shadows distinct while softening
    distant blockers without extra rays. On supported devices, dynamic shadows
    and lighting within 384 world units remain per-pixel while distant emissive
    lighting shares results in 2x2 quads. AO affects baked lighting but
    preserves emissive glow. A renderer restart is required. If the selected
...
vk_rt_ao::
    ...
    surfaces, clamped between 0 and 0.5. Nearby contacts receive most of the
    effect, while already-dark lightmap texels and glowmapped pixels are
    protected from excessive darkening. The baked atlas is reused when this
    value changes, so changes apply immediately without rebuilding the map.
    Value 0 disables the effect. Default value is 0.24.

vk_rt_emissive::
    Adds restrained, ray-shadowed direct lighting from opaque BSP surfaces
    marked with the Quake II ‘SURF_LIGHT’ flag. This is
    additive to baked lightmaps and attenuated by both the emitting and receiving
    surface angles. Light color is derived from each surface's glow mask or
    brightest base-texture pixels when no glow mask exists. Each face retains
    its strongest 12 possible sources; the shader chooses the strongest two at
    the fragment position and traces only significant contributions. Emissive
    shadows fade out for large nearby source faces to approximate area-light
    softness, and weak secondary shadows skip traversal while retaining their
    direct light. Influence lists remain in the world mesh so ordinary face
    batching stays available.
    The control uses a perceptual response that keeps low values visible.
    Changes apply immediately. Default value is 0.35; 0 skips surface-light
    evaluation and shadow rays.

Ray tracing preserves glowmap bloom when ‘gl_bloom’ is enabled. For a restrained

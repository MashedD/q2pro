...
vk_rt_caustics::
    ...
    Vulkan ray tracing is active, clamped between 0 and 1. Water uses a cool
    blue tint, slime green, and lava orange. Floors receive the strongest
    pattern while walls retain a softer contribution; the effect fades between
    512 and 1152 world units from the camera. It is procedural and adds no ray
    queries, texture samples, render passes, descriptors, or draw calls. Bright
    crests provide a restrained bloom contribution when ‘gl_bloom’ is enabled.
...
vk_rt_debug::
    ...
    black while the camera is outside water, slime, or lava. Value 0 restores

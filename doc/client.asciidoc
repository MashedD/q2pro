...
vk_rt_entity_specular::
    Controls dynamic-light specular highlights on eligible alias models while
    Vulkan ray tracing is active, clamped between 0 and 1. Animated MD2 and
    MD5 normals produce two-sided broad exponent-4 primary and focused
    exponent-24 clearcoat lobes, matching the unculled legacy model geometry.
    A mostly neutral static glint remains visible on strongly tinted skins. A
    bounded broad sheen contributes to scene color and stabilizes bloom from
    small and distant models without
    introducing the removed silhouette/rim treatment; bright skin details
    provide a restrained scene-color fallback on legacy models with unusable
    normals. A
    low-energy, light-tinted coverage floor keeps those models visible after
    bloom downsampling without restoring the old full-color model bloom. The
    existing static entity key light remains visible during ordinary play,
    while the strongest nearby dynamic light adds a colored glint. Entity
    effects use a separate additive overlay and do not access the acceleration
    structure, so the dynamic glint is deliberately not occlusion checked and
    cannot replace or darken the model's ordinary raster lighting. The held
    weapon is always eligible; other models use the overlay only within 512
    units to bound its draw cost. Entity effect controls use an explicitly
    aligned push-constant layout shared by the CPU and both alias shaders.
    Degenerate or
    non-finite animated normals skip the effect for that fragment instead of
    affecting its raster base color. Color headroom has a small floor for
    bright skins and combined highlight luminance is capped at 0.35.
    Translucent, shell, tracker, and ordinary fullbright models remain
    excluded; glowing bonus items remain eligible even when fullbright. Static
    highlights have no distance cutoff, while dynamic-light visibility remains
    limited to nearby entities. Highlight shape, the broad sheen, and the
    material fallbacks feed bloom capped at 0.45; the previous full-color
    whole-model RT bloom is removed. Entity effects issue no ray queries and
    add one overlay draw per eligible nearby model batch. Setting this to 0
    removes those overlay draws and leaves only the ordinary alias-model
    pipeline. Changes apply immediately.
...
vk_rt_debug::
    ...
    colored dynamic-shadow fringe amplified eight times. Value 12 blacks out
    BSP surfaces and shows eligible entity highlights amplified eight times.
    Value 0 restores

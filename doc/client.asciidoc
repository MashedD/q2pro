...
vk_raytracing::
    Enables the optional Vulkan ray-query path. The renderer uses ray queries
    for hard dynamic-light shadows and a single deterministic reflection ray
    on opaque world surfaces. A renderer restart is required. If the selected
...
‘roughness_override’, and ‘specular_scale’ from ‘materials/*.mat’ and the
map-specific ‘<mapname>.mat’. Unknown properties are ignored. Material names
may be comma-separated and ‘*’ in ‘texture_base’ expands to the material's

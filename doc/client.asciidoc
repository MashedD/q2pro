...
vk_raytracing::
    ...
    pixel lightmaps, glow maps, and bloom while using ray queries for hard
    dynamic-light shadows and stable, single-sample ambient occlusion on opaque
    world surfaces. Coplanar BSP faces share an AO direction, and occlusion
    strength falls smoothly with hit distance to avoid triangulation patches.
    On supported devices, dynamic shadows and lighting within
    384 world units remain per-pixel while distant static lighting and ambient
    occlusion share results in 2x2 quads. Ambient occlusion fades out between
    384 and 640 world units. It affects baked lighting but preserves emissive
    glow. A renderer restart is required. If the selected
...
vk_rt_ao::
    ...
    protected from excessive darkening. Value 0 skips ambient-occlusion rays.
    Changes apply immediately. Default value is 0.24.

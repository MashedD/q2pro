...
vk_raytracing::
    ...
    dynamic-light shadows on opaque world surfaces. A renderer restart is
    required. If the selected
...
vk_rt_emissive::
    ...
    additive to baked lightmaps. A renderer restart is required. Default value
    is 0.35; 0 disables surface-light contribution.
...
other geometry continues to use the normal raster path.

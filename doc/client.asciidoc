...
vk_raytracing::
    ...
    emitter data supplies only a luminance-neutral tint and a tightly gated
...
vk_rt_emissive::
    Controls restrained, ray-shadowed color from opaque BSP surfaces marked
    with the Quake II ‘SURF_LIGHT’ flag. Original lightmap luminance is never
    replaced or supplemented by this data. Visible emitters may instead add a
    subtle luminance-neutral tint and, when bloom is enabled, a localized bloom
    ...
    soft, stable visibility. Tint is capped at two percent of local luminance
    and 0.01 absolute intensity. Static bloom requires strong emitter radiance
    and is capped independently, preventing broad exposure changes. If baking
    fails, the per-frame two-source implementation supplies the same tint and
    bloom inputs. Influence lists remain in the world mesh so ordinary face
    batching stays available. Changes apply immediately without rebaking.
    Default value is 0.35; 0 removes tint and static RT bloom while the shared
    atlas remains available for ambient occlusion.
